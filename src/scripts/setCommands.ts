...
if (!TARGET_GUILD || !BOT_TOKEN || !BOT_ID || !ALLOWED_MANAGERS) {
  throw new Error('Cannot find one of: TARGET_GUILD, BOT_TOKEN, BOT_ID')
}

// Define the expected shape of the commands
const commands = [
  new SlashCommandBuilder().setDefaultPermission(false).setName('w-start').setDescription('Starts whale watcher'),
  new SlashCommandBuilder().setDefaultPermission(false).setName('w-stop').setDescription('Stops whale watcher'),
]
    .map(command => command.toJSON());

// Define async script
const main = async () => {
  console.log('Uploading new permissions')
